...
% Regenerate truth_trajectory.mat by running the T01 pipeline scripts.
    addpath(t01_dir);
    csv_candidates = { ...
    ...
    if isempty(csv_path)
        % Search wider
        d = dir(fullfile(t01_dir, '*.csv'));
        if isempty(d)
            error('test_imu_model:NoCSV', 'No RasAero CSV under %s', t01_dir);
        end
