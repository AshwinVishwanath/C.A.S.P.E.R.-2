...
% Source firmware reference: none (top-level integration glue).
    ...
    % Ensure the upstream T01 lib & its bus are available
    build_root = fileparts(here);
    t01_dir    = fullfile(build_root, 'T01_truth_pipeline');
    t01_lib    = fullfile(t01_dir,    'casper_sim_lib.slx');
    if ~isfile(t01_lib)
        error('build_casper_sim_phase0:NoT01Lib', ...
            'T01 library not found at %s. Run T01 build first.', t01_lib);
    end
    addpath(t01_dir);
    truth_bus = casper_truth_build_bus();
    assignin('base', 'TruthBus', truth_bus);

    % Close & delete prior version of OUR model (not T01's lib).
    if bdIsLoaded(model_name); close_system(model_name, 0); end
    if isfile(model_path); delete(model_path); end

    % Load T01 lib read-only (don't unlock, don't save).
