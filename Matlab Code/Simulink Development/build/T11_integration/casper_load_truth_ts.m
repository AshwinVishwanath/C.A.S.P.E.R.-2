...
%
% Behaviour:
%   - Looks for cached truth_trajectory.mat under build/T01_truth_pipeline/.
%     If missing, runs the T01 build chain (rasaero_ingest → resample →
...
%
% Inputs (name/value):
%   'Regenerate'  (default false) — force re-run of the T01 build chain
...
% smaller StopTime (e.g. 5.0 for the pad-only smoke) to keep base WS small.
    ...
    mat_path = fullfile(t01_dir, 'truth_trajectory.mat');

    if p.Results.Regenerate || ~isfile(mat_path)
        fprintf('[load_truth_ts] regenerating truth_trajectory.mat via T01 build...\n');
        raw    = casper_rasaero_ingest();          %#ok<NASGU>  writes to T01 dir
