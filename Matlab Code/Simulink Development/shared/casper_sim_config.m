...
%
% Side effects:
%   - Adds every Simulink Development task dir (truth, params, sensors/*,
%     nav/*, validation, integration, shared) to MATLAB path so all helper
%     functions (sensor models, EKF, attitude, frame switch, metrics)
%     resolve by name without further user setup.
%   - Loads casper_sensor_params (params/) into base workspace.
...
% Source firmware reference: none (sim-side wiring).
    ...
    % --- Paths ---------------------------------------------------------------
    here    = fileparts(mfilename('fullpath'));   % .../Simulink Development/shared
    simroot = fileparts(here);                    % .../Simulink Development
    repo_root = fileparts(fileparts(simroot));    %#ok<NASGU>
    ...
    for k = 1:numel(task_dirs)
        d = fullfile(simroot, task_dirs{k});
        if isfolder(d)
            addpath(d);
        end
    end
    ...
    cfg.Plots_OutDir = fullfile(simroot, 'integration', 'plots');
    cfg.Logs_OutDir  = fullfile(simroot, 'integration', 'logs');
    cfg.Data_OutDir  = fullfile(simroot, 'integration', 'data');
    ...
    fprintf('[sim_config] Seed=%u, StopTime=%.1f s\n', seed, stop_time);
end
