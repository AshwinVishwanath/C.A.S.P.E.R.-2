...
%   we approximate as a fixed 100 Hz sample rate per T04 spec section 5.5.
    ...
    here = fileparts(mfilename('fullpath'));         % .../sensors/baro
    simroot = fileparts(fileparts(here));            % .../Simulink Development
    if isempty(lib_path)
        % Canonical library lives in truth dir per truth convention.
        truth_dir = fullfile(simroot, 'truth');
        truth_dir = char(java.io.File(truth_dir).getCanonicalPath());
        lib_path  = fullfile(truth_dir, 'casper_sim_lib.slx');
    end

    % We also write a standalone copy under sensors/baro for storage / inspection.
    ...
    if ~evalin('base', 'exist(''Sim'', ''var'')')
        evalin('base', sprintf('run(''%s'')', strrep(fullfile(simroot, ...
            'params', 'casper_sensor_params.m'), '\', '/')));
    end
