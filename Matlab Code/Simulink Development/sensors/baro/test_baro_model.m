...
% baro_block subsystem from baro_block.slx.
    ...
    simroot_local = fileparts(fileparts(here));      % .../Simulink Development
    if ~evalin('base', 'exist(''Baro'',''var'')')
        evalin('base', sprintf('run(''%s'')', strrep( ...
            fullfile(simroot_local, 'params', 'casper_sensor_params.m'), '\', '/')));
    end
    addpath(fullfile(simroot_local, 'truth'));
