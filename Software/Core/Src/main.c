...
#include "app_main.h"       /* app_init(), app_tick(), extern driver instances */
#include "board_casper2.h"  /* casper_board_early_init() + BSP_* singletons   */
#include "radio_irq.h"      /* g_radio_dio0_flag, g_radio_dio1_flag (EXTI callback) */
...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
/* Compile-time sanity check lives in app_main.c where the modes are used. */
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
/* Driver-instance definitions moved to App/flight/app_main.c.
 * Extern declarations are in App/flight/app_globals.h (included via app_main.h). */
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* Early board bring-up: LED GPIO raw-register config + milestone LED1.
   * Moved to board_casper2.c so the register-level detail stays in the
   * board layer.  DWT cycle counter enable is also inside this call
   * (see USER CODE BEGIN SysInit comment below). */
  casper_board_early_init();
  /* USER CODE END Init */
  ...
  /* DWT cycle-counter enable moved into casper_board_early_init() (called
   * in USER CODE BEGIN Init above).  casper_board_early_init() runs both the
   * pre-clock LED GPIO setup and the post-clock DWT enable in one call.
   * The call must happen before SystemClock_Config() for the LED milestone,
   * but the DWT enable is safe to run at any point after reset — putting it
   * in the same function is harmless and keeps all early board bring-up in
   * one place. */
...
#endif
  /* USER CODE BEGIN 2 */
  /* All application init (sensor drivers, nav stack, FSM, pyro, radio,
   * logger, calibration dispatch) has moved to App/flight/app_main.c.
   * Call the portable entry point here. */
  app_init();
