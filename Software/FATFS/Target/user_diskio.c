...
/* W25Q512JV flash instance (defined in main.c) */
...
)
{
  /* USER CODE BEGIN STATUS */
    Stat = flash.initialized ? 0 : STA_NOINIT;
    return Stat;
...
)
{
  /* USER CODE BEGIN WRITE */
    ...
    for (UINT i = 0; i < count; i++) {
      ...
      if (w25q512jv_write(&flash, addr, buff + (i * SECTOR_SIZE), SECTOR_SIZE) != W25Q_OK)
        return RES_ERROR;
    }
...
)
{
  /* USER CODE BEGIN IOCTL */
    ...
    switch (cmd) {
    case CTRL_SYNC:
      /* No pending writes to flush (polling mode) */
      res = RES_OK;
      break;
