...
/* NOTE: get_le16 and get_le32 are provided by endian.h (static inline).
 *       The following helpers are only needed for the ASCII relay path. */
...
/* get_le16_signed: sign-extend the unsigned result from endian.h */
static int16_t get_le16_signed(const uint8_t *p)
{
    return (int16_t)get_le16(p);
}

/* get_le24: 3-byte little-endian unsigned (not in endian.h) */
static uint32_t get_le24(const uint8_t *p)
{
    return (uint32_t)p[0] | ((uint32_t)p[1] << 8) | ((uint32_t)p[2] << 16);
}

/* get_le32_signed: sign-extend the unsigned result from endian.h */
...
/*  for the 0x13 status heartbeat in COBS mode)                       */
/* ------------------------------------------------------------------ */
...
int ground_radio_cobs_send(const uint8_t *raw, int len)
{
     ...
     * Buffer is 39 + 3 = 42 bytes for safety margin.
...
void ground_radio_on_rx(void)
{
    ...
    /* Validate CRC-32: last 4 bytes of packet are the CRC */
    uint32_t payload_len = (uint32_t)(nb - 4);
    uint32_t computed    = crc32_hw_compute(s_rx_buf, payload_len);
    uint32_t received    = get_le32(&s_rx_buf[payload_len]);  /* endian.h */
    ...
    if (msg_id == MSG_ID_FAST && nb >= SIZE_FC_MSG_FAST) {
        ...
        *p++ = 0;
         ...
         * Approximations: ISA troposphere uses AGL altitude as MSL proxy;
         * vertical EKF velocity is used as the speed proxy; qbar clamped at u16 max. */
        {
            /* Assemble raw fields directly — avoids get_le* helpers that are
             * #ifndef GS_OUTPUT_COBS-guarded and unavailable in COBS mode. */
            ...
            const float Rs    = 287.05f;    /* J/(kg*K) specific gas const for air */
         ...
         * casper_quat_to_euler: e[0]=bodyZ=YAW, e[1]=bodyY=ROLL, e[2]=bodyX=PITCH
         * Wire encoding: i16 LE x0.1 deg, raw = round(deg * 10), clamped. */
        {
            float q_gs[4];
            quat_unpack_smallest_three(&s_rx_buf[8], q_gs);
            float e_gs[3];
            casper_quat_to_euler(q_gs, e_gs);
            long roll_raw  = lroundf(e_gs[1] * 10.0f);
            long pitch_raw = lroundf(e_gs[2] * 10.0f);
            long yaw_raw   = lroundf(e_gs[0] * 10.0f);
         ...
         * relay raw received packet COBS-framed — MC parses these natively. */
